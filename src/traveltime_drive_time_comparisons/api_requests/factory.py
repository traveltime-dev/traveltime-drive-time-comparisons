...
def initialize_request_handlers(providers: Providers) -> Dict[str, BaseRequestHandler]:
    def create_google_handler(provider: Provider):
        return GoogleRequestHandler(provider.credentials.api_key, provider.max_rpm, provider.api_endpoint)

    def create_tomtom_handler(provider: Provider):
        return TomTomRequestHandler(provider.credentials.api_key, provider.max_rpm, provider.api_endpoint)

    def create_here_handler(provider: Provider):
        return HereRequestHandler(provider.credentials.api_key, provider.max_rpm, provider.api_endpoint)
    ...
    def create_openroutes_handler(provider: Provider):
        return OpenRoutesRequestHandler(provider.credentials.api_key, provider.max_rpm, provider.api_endpoint)

    def create_mapbox_handler(provider: Provider):
        return MapboxRequestHandler(provider.credentials.api_key, provider.max_rpm, provider.api_endpoint)
